        ...
        match event {
            ...
            FrontendEvent::AuthorizedUpdated(map) => {
                ...
                self.pairing_attempts
                    .retain(|a| !authorized.contains_key(&a.fingerprint));
...
impl PairingCard {
    ...
    /// still live, otherwise the oldest live one. Live means not yet trusted,
    /// repeated within [`Self::STALE_AFTER`], and not `snoozed` (denied).
    pub fn show<'m>(
        &mut self,
        model: &'m AppModel,
        now: Instant,
        ...
        let live = |a: &&PairingAttempt| {
            !model.authorized.contains_key(&a.fingerprint)
