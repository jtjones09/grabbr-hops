...
hops build-check --repo /path/to/repo --strict   # exit 2 stale, 3 nothing compared
```
...
checkout at the path, a path below the top of a checkout or with no work tree, a
checkout with nothing committed, no git, git refusing the checkout, or a binary
built without a commit baked in. Any
...
directory, the default, is searched upward. An empty `HOPS_REPO` counts as unset.
